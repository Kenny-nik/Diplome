...
class ProfileAdmin(admin.ModelAdmin):
    ...
    показываем пользователя и превью аватара, никаких ссылок на несуществующие created_at/updated_at.
    """
...
class UserAdmin(BaseUserAdmin):
    ...
    - не используем list_filter по несуществующему полю is_librarian
    - признак «библиотекарь» показываем отдельным методом (по группе или атрибуту)
    """
    def is_librarian_flag(self, obj):
        # Если в модели есть булево поле/свойство is_librarian — используем его,
        # иначе проверяем принадлежность к группе "Библиотекарь"
    ...
    list_filter = ("is_active", "is_staff", "is_superuser")  # БЕЗ is_librarian
    search_fields = ("username", "email", "first_name", "last_name")
    ordering = ("-date_joined",)
    actions = (deactivate_users, activate_users, add_to_librarians_group)

    # Поля редактирования — только существующие стандартные атрибуты
    fieldsets = (
